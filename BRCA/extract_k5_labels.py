...
def main():
    ...
    # 为 mRNA 视图中的样本生成标签（common + mRNA-unique）
    ge_view = datasets[0]  # mRNA 视图
    ge_samples = ge_view.index.tolist()
    ge_orig_indices = [int(s.split("_")[1]) for s in ge_samples]
    ge_clusters = [orig_to_cluster[i] for i in ge_orig_indices]
    ...
    out_csv = os.path.join(BASE_DIR, "R", "brca_k5_labels.csv")
    ...
    out_ge_csv = os.path.join(BASE_DIR, "R", "brca_k5_labels_mRNA.csv")
    df_ge.to_csv(out_ge_csv, index=False)
    print(f"  mRNA视图标签已保存: {out_ge_csv}")
    print(f"  mRNA视图样本数: {len(df_ge)}")

    # 打印颜色参考
    brca_colors = ["#E64B35", "#4DBBD5", "#00A087", "#3C5488", "#F39B7F"]
    print("\n>>> 生存曲线颜色:")
    for i in range(5):
        print(f"  Cluster {i+1}: {brca_colors[i]}")
