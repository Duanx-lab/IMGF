...
def load_brca_data():
    """加载 BRCA 三组学数据 + 生存数据"""
    result = pyreadr.read_r(r"D:\Mysoftware\R\data\BRCA1.RData")

    ge = result["mydatGE"]      # mRNA: 628 × 25554
    me = result["mydatME"]      # 甲基化: 628 × 1000
    mi = result["mydatMI"]      # miRNA: 628 × 1046
    survival = result["survival"]  # fustat, futime, PAM50, pstage, age

    print(f"mRNA: {ge.shape}, 甲基化: {me.shape}, miRNA: {mi.shape}")
    print(f"生存数据: {survival.shape}")
    print(f"事件数: {int(survival['fustat'].sum())}/{len(survival)}")
...
# ============================================================
def split_three_omics_missing(data_list, survival, ratio=0.8, seed=42):
    """
    三组学均部分缺失：502 common + 42 unique/view = Union 628。
    ...
    # 三个组学各自保留 common + 各自的 unique
    d0 = build_view(data_list[0], u1, "ge")      # mRNA
    d1 = build_view(data_list[1], u2, "me")      # 甲基化
    d2 = build_view(data_list[2], u3, "mi")      # miRNA
...
def survival_analysis(cluster_labels, union_samples, union_to_orig, survival,
                       n_clusters, save_path, cluster_colors=None):
    """
    Kaplan-Meier 生存曲线 + log-rank 检验。

    对每一对簇做两两 log-rank test，报告全局显著性。
    """
    # 为每个样本分配簇标签
    sample_cluster = {}
    ...
    surv_df["futime"] = pd.to_numeric(surv_df["futime"], errors="coerce") / 30.44  # days → months
    surv_df["fustat"] = pd.to_numeric(surv_df["fustat"], errors="coerce")
    # 先分配 cluster 标签（基于原始 0..627 位置）
    ...
    # PAM50 参考
    mask0 = surv_df["PAM50"] == 0
    mask1 = surv_df["PAM50"] == 1
    pam50_pval = None
    if mask0.sum() > 0 and mask1.sum() > 0:
        lr_pam50 = logrank_test(
            surv_df.loc[mask0, "futime"], surv_df.loc[mask1, "futime"],
            surv_df.loc[mask0, "fustat"], surv_df.loc[mask1, "fustat"]
        )
        pam50_pval = lr_pam50.p_value
    ...
    # 右上角：多变量 log-rank p 值（科学计数法 + 上标）
    def _fmt_p(p):
        if p <= 0 or np.isnan(p) or p < 1e-15:
            return r"$p < 10^{-15}$"
    ...
    # 左上角：PAM50 参考
    if pam50_pval is not None:
        pam50_text = "PAM50 reference\n" + _fmt_p(pam50_pval)
        ax.text(0.02, 0.98, pam50_text, transform=ax.transAxes,
    ...
    if pam50_pval is not None:
        print(f"\n  === PAM50 生存差异（参考）===")
        print(f"    PAM50=0 (n={mask0.sum():.0f}) vs PAM50=1 (n={mask1.sum():.0f}): "
              f"p={pam50_pval:.6f} {'***' if pam50_pval < 0.001 else ''}")
...
def main():
    ...
    # K=5 聚类 + 生存分析
    BRCA_K5_COLORS = ["#E64B35", "#4DBBD5", "#00A087", "#3C5488", "#F39B7F"]
    n_clusters = 5
    ...
    p_val = survival_analysis(labels, union_samples, union_to_orig, surv,
                              n_clusters, save_path,
                              cluster_colors=BRCA_K5_COLORS)
